...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>DatLuvArisú chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "DatLuvArisú", HidePremium = false, IntroText = "Skill Issue⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("DatLuvArisú SCRIPT")
W:AddLabel("Discord:datyeudoi:3")
local Section = W:AddSection({
    Name = "Status"
})
...
C:AddButton({
    ...
    Callback = function(value)
        setclipboard("https://discord.com/invite/N38QTMupPZ)
    end
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "DatLuvArisú(Noitification)",
